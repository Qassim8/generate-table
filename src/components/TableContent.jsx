...
function TableContent() {
    ...
    const role = localStorage.getItem("userRule"); 
            ...
            selector: (row) => {
              ...
              return sessions.length > 0
                ? sessions.map((session) => (
                    session.status === "pending" && role === "admin"?<div key={session._id} className="text-sm py-3">
                      ...
                      <div className="text-gray-400">{session.classroom}</div>
                        ...
                        session.status === "approved" ? <div key={session._id} className="text-sm py-3">
                      <span className="font-semibold text-indigo-600">
                        {session.course}
                      </span>
                      <div className="text-gray-500">Dr. {session.teacher}</div>
